...
post '/wines' do
  ...
  response = RestClient.get "http://api.snooth.com/wines/?akey=" + ENV['API_KEY'] +"&ip=208.184.96.233&q=" + search_query1 + '&n=2' + search_query2
  ...
  wine_codes.each do |code|
      response = RestClient.get "http://api.snooth.com/wine/?akey=" + ENV['API_KEY'] + "&ip=208.184.96.233&food=1&id=" + code
      parsed_data = JSON.parse(response)
